...
string casein <arg("case","dakota_soga.in")>;


# outputs
file outdat         <arg("dat","results/out_soga.dat")>;
file outhtml        <arg("html","results/out_soga.html")>;
...
app (file pout, file perr, file dout, file derr, file outdat, file outhtml ) prepInputs (file[] openfoam, file[] dakota, file swiftconf) {
    bash "dakota/utils/prepInputs_soga.sh" "inputs/sweepParams.run" "inputs/elbowKPI.json" stdout=@pout stderr=@perr;
#	tree stdout=@pout stderr=@perr;
    bash "templatedir/start_docker_soga.sh" stdout=@dout stderr=@derr;
}
